                    ...
                    {[...brandFiles, ...brandFiles].map((file, index) => (
                        ...
                        >
                            <div className="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                                <img 
                                    src={`/images/${file}`}  {/* यहां सही path दिया */}
                                    alt={`Brand ${index + 1}`}
                                    className="brand-image h-16 w-auto object-contain transition-all duration-300"
                                    draggable={false}
                                    loading="lazy"
                                    onError={(e) => {
                                        console.error(`Image not found: /images/${file}`);
                                        e.target.onerror = null;
                                        e.target.src = '/images/placeholder.png'; // Optional placeholder
                                    }}
                                />
                            </div>
                        </div>
