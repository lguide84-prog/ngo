...
function ProductCard({ product }) {
  ...
  return product && (
    <div
      ...
      className="border border-gray-500/20 rounded-md px-3 md:px-4 py-2 bg-white w-full max-w-[16rem] hover:shadow-md transition-shadow cursor-pointer"
    >
      {/* Category Badge */}
      <div className="mb-2 flex flex-wrap gap-1">
        {hasSubcategory && product.subCategory ? (
          // Show subcategory badge if available
          <span className={`px-2 py-1 rounded text-xs font-medium ${getSubCategoryColor(product.category, product.subCategory)}`}>
            {getSubCategoryEmoji(product.category, product.subCategory)} {product.subCategory}
          </span>
        ) : (
          // Show only category badge if no subcategory
        ...
        {product.inStock ? (
          ...
          <span className="px-2 py-1 rounded text-xs font-medium bg-red-50 text-red-800 border border-red-200">
            ❌ Out of Stock
          </span>
        )}
      </div>
      ...
      <div className="text-gray-500/60 text-sm mt-2">
        {/* Show category name below badges for clarity */}
        {hasSubcategory && product.subCategory && (
