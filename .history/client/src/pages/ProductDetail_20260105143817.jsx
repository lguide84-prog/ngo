...
const ProductDetail = () => {
  ...
  // Function to get emoji for weight unit
  const getWeightUnitEmoji = (unit) => {
    const unitEmojiMap = {
      "kg": "⚖️",
      "g": "⚖️",
      "l": "💧",
    ...
    return unitEmojiMap[unit] || "📦";
  };

  // Function to format weight display
  const formatWeightDisplay = () => {
    if (!product.weightValue && !product.weightUnit) return null;
    
    if (product.weightValue && product.weightUnit) {
      return `${product.weightValue} ${product.weightUnit}`;
    }
    ...
    <div className="mt-12">
      ...
      <div className="flex flex-col md:flex-row gap-16 mt-4">
        ...
        <div className="text-sm w-full md:w-1/2">
          ...
          {formatWeightDisplay() && (
            <div className="mt-3">
              <div className="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-800 rounded-lg border border-gray-300">
                <span className="font-medium mr-2">{getWeightUnitEmoji(product.weightUnit)}</span>
                <span>
                  {formatWeightDisplay()}
                </span>
              </div>
          ...
          <div className="flex items-center gap-0.5 mt-6">
            {Array(5).fill('').map((_, i) => (
              product.rating > i ? (
                <img src={assets.star_icon} alt="star" className="w-5 h-5" key={i} />
              ) : (
                <svg width="20" height="20" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg" key={i}>
