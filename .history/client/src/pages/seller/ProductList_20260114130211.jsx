...
function ProductList() {
  ...
  const openEditModal = (product) => {
    ...
    setEditData({
      ...
      shippingCharges: product.shippingCharges || '0', // ✅ New field
  ...
  const updateProduct = async (e) => {
    ...
    try {
      ...
      const productData = {
        ...
        shippingCharges: Number(editData.shippingCharges) || 0, // ✅ Include shipping charges
  ...
  const shippingOptions = [0, 80, 100, 120, 200, 300]; // ✅ Shipping charge options

  // Calculate total price with GST and shipping
  const calculateTotalPrice = (product) => {
    ...
    const totalPrice = priceWithGST + shipping;
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {products && products.length > 0 ? (
                  products.map((product) => {
                    const priceDetails = calculateTotalPrice(product);
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-1">
                            ...
                            <div className="text-xs text-gray-500">
                              +GST: ₹{priceDetails.gstAmount.toFixed(2)}
                            </div>
                            <div className="text-xs text-gray-500">
                              +Shipping: ₹{priceDetails.shipping.toFixed(2)}
                            </div>
                            <div className="text-xs font-medium pt-1 border-t border-gray-200">
                              Total: ₹{priceDetails.totalPrice.toFixed(2)}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${
                            product.shippingCharges === 0 ? 'bg-green-100 text-green-800' :
                            product.shippingCharges === 80 ? 'bg-blue-100 text-blue-800' :
                            product.shippingCharges === 100 ? 'bg-yellow-100 text-yellow-800' :
                            product.shippingCharges === 120 ? 'bg-orange-100 text-orange-800' :
                            product.shippingCharges === 200 ? 'bg-purple-100 text-purple-800' :
                            'bg-red-100 text-red-800'
                          }`}>
                            <Truck className="w-3 h-3" />
                            ₹{product.shippingCharges || 0} {product.shippingCharges === 0 ? '(Free)' : ''}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-gray-50 p-3 rounded-lg">
                          <div className="text-xs font-medium text-gray-700 mb-2">Base Price</div>
                          <div className="font-bold text-green-700 text-lg">
                            ₹{product.offerPrice}
                          </div>
                        </div>
                        <div className="bg-gray-50 p-3 rounded-lg">
                          <div className="text-xs font-medium text-gray-700 mb-2">Shipping</div>
                          <div className={`font-bold ${product.shippingCharges === 0 ? 'text-green-700' : 'text-blue-700'} text-lg`}>
                            ₹{product.shippingCharges || 0} 
                            {product.shippingCharges === 0 && ' (Free)'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <div className="text-xs font-medium text-gray-700 mb-1">GST</div>
                          ...
                          <div>
                            <div className="text-xs font-medium text-gray-700 mb-1">GST Amount</div>
                            <div className="font-medium text-gray-900">
                              ₹{priceDetails.gstAmount.toFixed(2)}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gradient-to-r from-green-50 to-blue-50 p-4 rounded-lg border border-green-200">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-xs text-gray-500">Includes GST + Shipping</div>
                          </div>
                        ...
                        <div className="mt-2 pt-2 border-t border-green-200">
                          <div className="grid grid-cols-3 gap-2 text-xs text-gray-600">
                            <div>Base: ₹{product.offerPrice}</div>
                            <div>GST: ₹{priceDetails.gstAmount.toFixed(2)}</div>
                            <div>Ship: ₹{priceDetails.shipping.toFixed(2)}</div>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <form onSubmit={updateProduct} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      value={editData.shippingCharges || '0'}
                      onChange={(e) => setEditData({...editData, shippingCharges: e.target.value})}
                      className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary focus:border-primary transition"
                      required
                    >
                      {shippingOptions.map(option => (
                        <option key={option} value={option}>
                          ₹{option} {option === 0 ? "(Free Shipping)" : ""}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  {editData.offerPrice && (
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                      <p className="text-sm font-medium text-gray-700 mb-3">Price Preview:</p>
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          <span>GST ({editData.gstPercentage || 5}%):</span>
                          <span>₹{(Number(editData.offerPrice) * Number(editData.gstPercentage || 5) / 100).toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span>Shipping:</span>
                          <span className={Number(editData.shippingCharges) === 0 ? "text-green-600" : ""}>
                            ₹{Number(editData.shippingCharges || 0).toFixed(2)} {Number(editData.shippingCharges) === 0 ? "(Free)" : ""}
                          </span>
                        </div>
                          ...
                          <div className="flex justify-between font-bold">
                            ...
                            <span className="text-green-700">
                              ₹{(
                                ...
                                (Number(editData.offerPrice) * Number(editData.gstPercentage || 5) / 100) + 
                                Number(editData.shippingCharges || 0)
                              ).toFixed(2)}
                            </span>
                          </div>
                        </div>
