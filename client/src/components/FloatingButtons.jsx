// FloatingButtons.jsx - Aayushman Bhava Natural Living Pvt. Ltd.
import React from 'react';

function FloatingButtons() {
  const phoneNumber1 = "917208659290";
  const phoneNumber2 = "917715011000";
  const whatsappNumber = "917208659290";
  ...
  // Phone call click handler (calls first number)
  const handlePhoneClick = () => {
    window.location.href = `tel:${phoneNumber1}`;
  };

  // Alternative: Show both options on click
  const handlePhoneClickWithOptions = () => {
    const userChoice = window.confirm(`Choose number to call:\n\n1. +91 72086 59290\n2. +91 77150 11000\n\nClick OK for first number, Cancel for second`);
    if (userChoice) {
      window.location.href = `tel:${phoneNumber1}`;
    } else {
